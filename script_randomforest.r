...
for(i in 1:k){
  ...
  train_root <- random_forest(training_data, T = 9 , m = round((ncol(dataset1) - 1) * 0.40),
                              N = round(0.55 * nrow(dataset1)))
  predicted_label <-(as.integer(rf.classify
                                (test_data, train_root)))
  
  true_label <- as.integer(test_data$label)
  
  #evaluating the results of the random function 
